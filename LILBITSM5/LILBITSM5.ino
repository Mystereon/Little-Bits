      ...
      void loop() {
        ...
        for (int gen = 0; gen < gens; gen++)
        {
          if (digitalRead(M5_BUTTON_HOME) == LOW) {
             gen = 0; M5.Lcd.setTextSize(2);  M5.Lcd.setTextColor(TFT_WHITE, RED); M5.Lcd.setCursor(35, 27);
            M5.Lcd.println(F("  NUKE  ")); delay(200); rcolor = random(0xFFFF);runt--;initGrid(); int GEN_DELAY =  20;
          }
...
void setres(){                           extern const unsigned char gImage_002[];
                                        ...
                                        M5.Lcd.println(res );
                                        if (digitalRead(M5_BUTTON_RST) == LOW) { res++; rcolor == random(0xFFFF); delay(200); }
                                        if (res == 5) { res = 1;}
                                        CELLXY == res;
                                        if (res ==1) { viewx = 120; viewy = 80;gens = 999;}
                                        if (res ==2) { viewx = 60; viewy = 40;gens = 999;}
